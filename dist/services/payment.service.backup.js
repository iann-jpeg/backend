...
async;
initiateMpesaStkPush(phone, string, amount, number, transactionId, string);
{
...
async;
initiatePaypalPayment(amount, number, transactionId, string, email, string);
{
...
async;
simulatePaymentProcessing(paymentMethod, string);
Promise < boolean > {
...
async;
handleMpesaCallback(callbackData, payment_dto_1.PaymentCallbackDto);
{
    return this.handlePaymentCallback(callbackData, 'mpesa');
}
async;
handlePaypalCallback(callbackData, payment_dto_1.PaymentCallbackDto);
{
    return this.handlePaymentCallback(callbackData, 'paypal');
}
async;
handleCardCallback(callbackData, payment_dto_1.PaymentCallbackDto);
{
    return this.handlePaymentCallback(callbackData, 'card');
}
async;
handlePaymentCallback(callbackData, payment_dto_1.PaymentCallbackDto, expectedMethod, string);
{
    try {
        const payment = await prisma.payment.findFirst({
...
//# sourceMappingURL=payment.service.backup.js.map