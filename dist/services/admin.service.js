...
let AdminService = class AdminService {
    ...
    async getDashboardData() {
        ...
        try {
            ...
            return {
                ...
                data: {
                    stats: {
                        totalUsers: ((_a = userStats.data) === null || _a === void 0 ? void 0 : _a.total) || 0,
                        totalClaims: ((_b = claimsStats.data) === null || _b === void 0 ? void 0 : _b.total) || 0,
                        totalPayments: ((_c = paymentStats.data) === null || _c === void 0 ? void 0 : _c.totalPayments) || 0,
                        totalRevenue: ((_d = paymentStats.data) === null || _d === void 0 ? void 0 : _d.totalRevenue) || 0,
                        growthRate: 0
                    },
    ...
    async getComprehensiveDashboardStats() {
        try {
            ...
            const [totalUsers, totalClaims, totalQuotes, totalConsultations, totalOutsourcingRequests, totalPayments, totalDiasporaRequests, pendingClaims, pendingConsultations, monthlyUsers, monthlyClaims, monthlyQuotes, monthlyConsultations, monthlyOutsourcingRequests, monthlyPayments, monthlyDiasporaRequests, lastMonthUsers, lastMonthClaims, lastMonthQuotes, lastMonthConsultations, totalRevenue, monthlyRevenue, lastMonthRevenue, avgPaymentAmount, recentClaims, recentConsultations, recentOutsourcingRequests, recentPayments, recentDiasporaRequests] = await Promise.all([
                    ...
                    allSubmissions: {
                        claims: recentClaims.map(claim => {
                            ...
                            return ({
                                ...
                                clientEmail: ((_a = claim.user) === null || _a === void 0 ? void 0 : _a.email) || claim.submitterEmail || 'N/A',
                        ...
                        payments: recentPayments.map(payment => {
                            ...
                            return ({
                                ...
                                clientEmail: ((_a = payment.user) === null || _a === void 0 ? void 0 : _a.email) || payment.email || 'N/A',
    ...
    async getSystemMetrics() {
        try {
            ...
            const [totalUsers, totalClaims, totalQuotes, pendingClaims, activeUsers, recentSignups, claimsThisMonth, quotesThisMonth] = await Promise.all([
                this.prisma.user.count(),
                this.prisma.claim.count(),
                this.prisma.quote.count(),
                this.prisma.claim.count({ where: { status: 'PENDING' } }),
    ...
    async getAllClaims(page = 1, limit = 50, status, search) {
        try {
            const skip = (page - 1) * limit;
            const where = {};
            if (status) {
    ...
    async downloadDocumentById(documentId, res) {
        try {
    ...
    getMockDocumentById(documentId) {
        const mockDocuments = {
            1: {
                id: 1,
    ...
    async getAllConsultations(page = 1, limit = 50, status, search) {
        try {
            const skip = (page - 1) * limit;
            let where = {};
            if (status) {
    ...
    async scheduleMeeting(consultationId, meetingData) {
        try {
            const consultation = await this.prisma.consultation.findUnique({
                where: { id: consultationId },
                include: {
    ...
    async sendWhatsAppMeetingDetails(consultationId, data) {
        ...
        try {
            ...
            const userPhone = (_b = (_a = consultation.user) === null || _a === void 0 ? void 0 : _a.profile) === null || _b === void 0 ? void 0 : _b.phone;
            if (!userPhone) {
                return {
                    success: false,
            ...
            let whatsappMessage = data.message || `Hello ${((_d = (_c = consultation.user) === null || _c === void 0 ? void 0 : _c.name) !== null && _d !== void 0 ? _d : 'Client')},\n\nYour consultation meeting has been scheduled:\n\n📅 Date: ${meetingDate}\n⏰ Time: ${meetingTime}\n⏱️ Duration: ${consultation.duration || 60} minutes`;
            ...
            return {
                success: true,
                data: {
                    recipient: (_h = (_g = consultation.user) === null || _g === void 0 ? void 0 : _g.name) !== null && _h !== void 0 ? _h : 'Client',
                    phone: userPhone,
    ...
    async getContentStats() {
        try {
            const [totalUsers, totalClaims, totalQuotes, totalConsultations, totalOutsourcing, totalResources, totalPayments] = await Promise.all([
                this.prisma.user.count(),
                this.prisma.claim.count(),
                this.prisma.quote.count(),
                this.prisma.consultation.count(),
    ...
    async generateReport(type, dateRange) {
        try {
            const startDate = (dateRange === null || dateRange === void 0 ? void 0 : dateRange.from) ? new Date(dateRange.from) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
            const endDate = (dateRange === null || dateRange === void 0 ? void 0 : dateRange.to) ? new Date(dateRange.to) : new Date();
            let data = {};
            switch (type) {
                case 'users':
    ...
    async getPaymentTrendsData(startDate) {
        try {
    ...
    async getTopProductsData(startDate) {
        try {
    ...
    async getPaymentStats() {
        try {
            const now = new Date();
            const firstDayOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
            const [totalRevenue, monthlyRevenue, totalPayments, monthlyPayments, avgPaymentAmount] = await Promise.all([
                this.prisma.payment.aggregate({
                    _sum: { amount: true }
                }),
                this.prisma.payment.aggregate({
    ...
    async exportData(dataType, format = 'json', limit = 1000, skip = 0) {
        try {
            let data = [];
            switch (dataType) {
                case 'users':
    ...
    async getAllOutsourcingRequests(page = 1, limit = 50, status, search) {
        try {
            const skip = (page - 1) * limit;
            const where = {};
            if (status) {
    ...
    async getOutsourcingStats() {
        try {
            const now = new Date();
            const firstDayOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
            const [totalCount, monthlyCount, statusCounts] = await Promise.all([
                this.prisma.outsourcingRequest.count(),
                this.prisma.outsourcingRequest.count({
                    where: { createdAt: { gte: firstDayOfMonth } }
                }),
    ...
    async restoreSystemBackup(backupId) {
        try {
